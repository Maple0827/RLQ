...
class RLQ:
    ...
    def train(self, training_set):
        ...
        while iteration < self.num_epoch:
            ...
            for req in training_set:
                ...
                if req.graph['type'] == 0:
                    ...
                    if len(node_map) == req.number_of_nodes():
                        ...
                        reward = Evaluation.uti_to_qos(sub_copy, req, link_map,'RLQ')
                        if reward != -1:
                            self.evaluation.collect(sub_copy, req, link_map,'RLQ')
                            # with open('ac_loss2.txt', 'a') as f:
                            #     f.write("%-10s\t%-20s\t%-20s\n" % (req.graph['time'], self.evaluation.acc_ratio,
                            #                                        self.evaluation.average_qos_loss))
                            epx = np.vstack(xs)
            ...
            # with open('ac_loss1.txt', 'a') as f:
            #     f.write("%-10s\t%-20s\t%-20s\n" % (iteration, self.evaluation.acc_ratio,
            #                                        self.evaluation.average_qos_loss))
