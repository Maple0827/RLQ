...
class Analysis:
    ...
    def __init__(self, result_dir):
        ...
        self.metric_names = {'acceptance ratio': 'Acceptance Ratio',
                             ...
                             'node cpu utilization': 'Average Node CPU Utilization',
                             'node queue utilization': 'Average Node QUEUE Utilization',
                             'link utilization': 'Average Link Utilization',
                             'qos loss': 'Average QoS Loss'}
    ...
    def save_epoch(self, epoch, acc, loss, runtime):
        """保存不同采样次数的实验结果"""

        filename = self.result_dir + 'epoch.txt'
        with open(filename, 'a') as f:
            f.write("%-10s\t%-20s\t%-20s\t%-20s\n" % (epoch, acc, loss, runtime))
    ...
    def read_result(self, filename):
        ...
        t, acceptance, revenue, cost, r_to_c, node_stress, que_stress, link_stress,qloss = [],[],[], [], [], [], [], [], []
        for line in lines:
            a, b, c, d, e, f, u, g, q = [float(x) for x in line.split()]
            t.append(a)
            acceptance.append(b)
            revenue.append(c / a)
            cost.append(d / a)
        ...
        return t, acceptance, revenue, cost, r_to_c, node_stress, que_stress, link_stress, qloss
    ...
    def draw_acc_loss(self):
        time, acc, loss = [], [], []
        with open("ac_loss2.txt") as f:
            lines = f.readlines()
            for line in lines:
                t, ac, los = [float(x) for x in line.split()]
                time.append(t)
                acc.append(ac)
                loss.append(los)
        plt.figure()
        ...
        plt.xlabel("time", fontsize=12)
        # plt.legend(['acceptance ratio','average_qos_loss'],loc='lower right', fontsize=12)
        plt.show()
